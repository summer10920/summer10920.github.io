...
Angular 通常是指 Angular 2+以上的版本（與早期的 Angular 1 版本完全不同，又稱呼第一代叫 AngularJS)，是由 AngularJS 團隊於 2016 年重新改寫並由 Google 所主導開發的 JavaScript 框架，與 React 相同都是採用組件 Component-based 來進行觀念導向設計，不像 VueJS 採用 MVVM（Model 資料管理、View 畫面顯示、ViewModal 溝通橋梁）觀念去區分細節，而是整個融合在 Component 整個零件內。
