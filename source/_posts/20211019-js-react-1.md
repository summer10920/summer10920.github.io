...
```
## Component 組件
由於 React 採用組件導向的框架，設計方式就是先建立組件接著利用組件來顯示頁面結果。定義組件的作法基礎方式就是採用純 JavaScript 函式來構成，而 React 的組件是採 ES6 的 class 來定義（透過繼承 React.Component 獲得原型），不過最近新觀念是使用 Hook，未來另起篇幅介紹。
...
>設計 Component 組件命名時必需一定是大寫開頭，這是避免在 JSX 內編寫時與小寫開頭的 HTML 元素混淆衝突，例如`<Div />`與`<div />`前者是組件後者是 HTML 標籤。
...
> props 本身就是一種函式內的傳遞值因此是不可改的（無法在組件內試圖修改 props)，React 遵循 Pure function 觀念只能對 props 限制僅讀取用。如果需要雙向更新的用法可改用 State 方式 ( 唯獨 class 而 function 沒有 )。
...
### 列表化組件
將前面的 map 改成都由 Component 來負責輸出，外面只需要提供列表清單並透過 props 來傳達給組件。
