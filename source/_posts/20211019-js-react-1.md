...
```js src/index.js
import {Component} from 'react'; //只拿Component
import {render} from 'react-dom';//只拿render
...
const elementC = <WelcomeC/>;
render(elementC, document.querySelector('#demo2')); //這裡是react-dom的render
```
...
**解決方式 3**
想辦法變成是透過`this.setState()`來觸發，因此調整JSX的指令，將event的值更改為觸發這個句話。
```js
class Clock extends React.Component {
  constructor(props) {
    super(props);
  ...
  render() {
    //調整event handle 透過箭頭匿名函式，是執行stopTime()
    return (
      <div>
        <FormatDate date={this.state.date} />
        <button onClick={() => this.stopTime()}>STOP</button>
...
**解決方式 4**
其實為方法3與方法1之合併版本，在JSX上的函式名稱做`bind(this)`。
