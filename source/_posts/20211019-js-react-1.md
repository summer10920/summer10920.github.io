...
```
接著在設計 component 當下，需要透過 React 來 createElement 創造 HTML 元素，以及對這個元素進行 HTML 編寫。最後透過虛擬 DOM 指定哪個元素跟元件並渲染到哪個實體 DOM。
...
```
設計元件時，就不用 React 的 createElement，直接將 JSX 寫在元件內即可。而虛擬 DOM 當下只需要直接寫元件並渲染到實體 DOM 上，因為 HTML 元素透過 JSX 直接寫在元件內。
```js custom.js
'use strict';
...
```
## Component 元件
由於 React 採用元件導向的框架，設計方式就是先建立元件接著利用元件來顯示頁面結果。定義元件的作法基礎方式就是採用純 JavaScript 函式來構成，而 React 的元件是採 ES6 的 class 來定義（透過繼承 React.Component 獲得原型），不過最近新觀念是使用 Hook，未來另起篇幅介紹。
...
將元件輸出到畫面上的做法如前面提到的，透過`ReactDOM.render（元件物件，目標 NODE)`來完成，元件的物件寫法為`<NAME/>`來代表。
...
如果這個元件有提供屬性參數會以 props 物件方式來保留，用途廣泛的傳遞資料於內外存取屬性。例如我們對外部元件設定添加`myname=Loki`，便能在內部元件利用此 props 來獲得進行應用。舉例使用 JSX 的`{}`表達式來插入 props 物件之變數：
...
> props 本身就是一種函式內的傳遞值因此是不可改的（無法在元件內試圖修改 props)，React 遵循 Pure function 觀念只能對 props 限制僅讀取用。如果需要雙向更新的用法可改用 State 方式 ( 唯獨 class 而 function 沒有 )。

### 組合 Component
小元件本身可重複利用在另一個大元件內利用 JSX 編寫重複使用，再透過 props 獲得差異性的顯示。首先這裡創造一個 MyApp 作為新元件並嘗試在 JSX 上使用 Welcome 元件。
...
//<h1>Hello, Loki!!</h1>
```
通常 React 會用這樣的方式在最外層自訂命名 app 代表整個組合式 Componet，將類似 Button 這樣的小元件寫在一起。

### 分解 Component
試著將下面單一式元件改成組合式元件。如果需要可偷偷增加`console.log`來檢查`this.props`內容物。
...
1. 首先最小單位的開始處理，整體最內部的`img.Avatar`下手。抽取出來獨立一個 Avatar 元件，而原本 Comment 元件的該處改成元件標籤。這裡故意使用 user 代表為 Comment 提供給 Avatar 專用的 props 變數獨立名稱不受影響。
```js src/index.js
class Avatar extends React.Component { //頭像元件
  render() {
...
```
2. 在 Avatar 的上層為 UserInfo，也進行進行抽離並將原本的 Avatar 元件一同搬移至另一元件內，開始變成組合式元件。
```js scr/index.js
class Avatar extends React.Component { //頭像元件
  render() {
...
class UserInfo extends React.Component { //用戶元件
  render() {
...
```
3. 完成後整體代碼如下，抽離成組合式元件在大型應用上常見到，但不是什麼都要抽離成元件。經驗上若有些 UI 會重複使用或是結構本身過於複雜，組合式元件下重複利用是很好的組合方式。
```js src/index.js
import React from 'react';
import ReactDOM from 'react-dom';
...
class UserInfo extends React.Component { //用戶元件
  render() {
    return (
      <div className="UserInfo">
        <Avatar user={this.props.author} />  {/* 引用 Avatar 元件 */}
        <div className="UserInfo-name">
          {this.props.author.name}
        </div>
      </div>
...
class Comment extends React.Component { //最大元件
  render() {
    return (
      <div className="Comment">
        <UserInfo author={this.props.author} /> {/* 引用 UserInfo 元件 */}
        <div className="Comment-text">
          {this.props.text}
        </div>
        <div className="Comment-date">
...
### Function 轉 Class
如果選擇請盡量使用 Class 方式來建立元件，只有 Class 才能提供 State 與生命週期這方面的特性。欲將 Function 轉為 Class 有以下步驟：
...
前面幾例介紹已將官方手冊的 function 改寫成 Class 可自參考兩者差異，這裡再追述稍早的範例進行轉換練習。然而這個 tick 函式不算是 React 的元件，僅只是普通函式將 JSX 透過 render() 渲染輸出。
...
先將適合做成的元件建立起來，而時間物件從外面產生透過 props 傳入到元件內。
...
接著將 Clock 的 function 元件改成 Class 元件。
...
### 區間狀態 Local state
state 是 Class 型元件才有的特性（另外還有生命週期等特性），前例的 Date 物件 是從 props 傳遞進去不可改，因此每次都要重新執行該元件。這裡由元件內部的 state 變數來負責記錄 Date 物件（才能在元件內進行修改值），另外改變 setInterval 從外部方式 render 方式改由生命週期負責更新。

1. 首先將 JSX 內的 props 改成 state。
2. 元件 class 宣告建構子 constructor 初始化 this.state 為 Date 物件。
...
組合式元件其各自的元件 state 是各自獨立存在 Local 互不影響，可使用瀑布的觀念透過將來自上層的 state 值，利用 props 傳遞給下層。但注意的是這是單向，你無法將下層的 state 傳遞給上層。範例中將 Clock 分解出一個時間文字格式。原本為將改成這樣：
...
class Clock extends React.Component { //上層，將 state 值以屬性提供給下層元件
  constructor(props) {
    super(props);
    this.state = { date: new Date() };
  }
...
生命週期是指當一個元件進行 Render 到實體 DOM 上時的掛載 Mount 動作開始，直到這個 DOM 被移除後進行卸載 Unmount 而結束。React 提供了多種週期操作方法，例如`componentDidMount()`與`componentWillUnmount()`能在觸發掛載與卸載時進行自訂工作。

範例中我們將 setInterVal 寫在`componentDidMount()`上，也就是當該元件被掛載到畫面上時會觸發這裡的作業。
...
最後，元件是可以重複被引用在大 Class 底下，多增加一個 Class 來重複引用 Clock，整體如下：
...
###  因此消失的 this
使用 Class 來設計元件，我們會利用 state 來進行動態顯示。前面出現範例，欲對 state 修改是沒有問題的。
...
1. 設計中元件（邏輯用）來判斷選擇哪個小元件 (UI 用），其後大總成輸出時，只需使用該元件即可。
2. 先使用變數指定判斷為哪個小元件 (UI 用），其後大總成輸出時元件，只需使用該變數即可。

```js
import { Component } from 'react';
import ReactDOM from 'react-dom';

/******** 透過 Greeting 元件來判斷選擇哪個元件標籤*******/
...
### 控制 Component 不輸出渲染
我們可利用 return null 的特性要求元件不進行渲染行為，達到該元件具備何時才會 render 之機制。
...
### 列表化元件
將前面的 map 改成都由 Component 來負責輸出，外面只需要提供列表清單並透過 props 來傳達給元件。
...
```
雖然正常顯示，但透過瀏覽器 Console 錯誤訊息會提示你的元件內的子項目資料應該要提供 key 去指定。因此調整一下 map 內的動作提供 key 屬性。但然而這裡因為 key 你根本用不到但必需要存在。
...
### Key 的出現位置
key 的指定處是落在於一陣列內的 JSX 元素，才能當 render 陣列時進行處理 key 的規劃。以下例子為組合 Component 下的示範說明：
...
## 表單設計
透過 html 表單元素能跟用戶進行資料處理，搭配 state 與 event 可以即時性雙向綁定作業，讓方便性提高不少。在 React 實施雙向綁定的方式稱呼為 Controlled Component 受控元件，讓用戶輸入完成動態更新 state 值而透過 setstate() 達到。但也不是什麼都能控制，例如 file 只能用戶來進行修改，無法透過 JavaScript 來反向修改（僅唯讀），這一類型又稱呼為 uncontrolled component 不受控元件。
...
```
> `input[type=file]`本身是唯獨屬性，React 只歸類為 uncontrolled component 不受控元件無法雙向綁定。
...
相較於 Controlled Component 設計是十分繁瑣的，尤其是每個元素都要設定事件函式動作加入 state 規劃。如果從某舊專案轉換或其他函式庫整合會很麻煩。因此可以使用 Uncontrolled Component 非受控元件，也就是捨棄 React 的 state 來處理，使這些表單值改由 JavaScript 本身 DOM 來處理。設計 Uncontrolled Component 最大的重點是沒有 state 而是透過 ref 獲得 DOM 的資料。
...
非受控元件採用 DOM 原本值的來源，整體代碼較短但效能較差，但能較容易與其他函式庫進行整合。如果可以還是盡量改用受控元件來設計表單。

#### 預設值
非受控元件本身 React 不會協助處理，但你可要求 React 幫忙指定預設值之後就不會管這些值處理。透過 JSX 元素之屬性`defaultValue` (Input,textarea,select) 或`defaultChecked` (checkbox, radio) 達到。
...
## 提升 state
前面提到在組合式元件應用上，state 本身會有單向資料流的特性。

此例為透過用戶輸入攝氏溫度值來回饋適合之文字。為設計兩個上下元件 Calculator 與 BoilingVerdict，透過 Calculator(parent) 的 state 提供給 BoilingVerdict(child) 的 props 來進行分解。
...
然後，改一下內容規劃成兩個 input，我們需要兩個類似的 input 提供華氏與攝氏的輸入。因此 Calculator 元件內的 input 與 event 相關代碼另外分解出來做成 TemperatureInput 元件
...
//整個畫面輸出，透過 props 使兩個 TemperatureInput 元件有不同應用
class Calculator extends Component {
  render() {
    return (
      <div>
...
現在還差 2 種溫度的轉換計算，。並設計具備檢查（有效輸入）、引用公式、後續處理（小數點處理）的主要轉換之函式。我們編列在 Calculator 元件內稍晚在解釋如何應用這些公式轉換函式
...
現在最大的重頭戲，如何讓兩個 TemperatureInput 元件各自的 local state 能彼此互相分享。這是不可逆流的單向資料流，解決方法便是透過提升 state 將綁定轉移到上層的 Calculator 元件（祖先），由父元件負責管理 state，而子元件負責 event 事件的傳達。我們希望兩個爛位的值來自同一個 state，這樣才能在 state 更動時兩個欄位也是動態來自同出處，使得形成反向資料流。

1. 將子元件的 local state 轉移到父元件，由父元件來負責 local state 透過 props 來提供給子元件。
2. 原本的 state 提升到父元件去了，因此子元件原本的 this.state 寫法改成 this.props
3. 子元件的 setState 之函式先複製一份到父元件去並替換函式名稱，父元件讓可以控制他自己的 local state。
4. 子元件原本的 event 事件是要控至自己的 setState，現在跟畫面有關的 state 已經在父元件內，子元件是不可能控制父元件的 state。但可以透過 props 請父元件將他的 setState 之函式傳過來。讓子元件提供函式參數在父元件的函式上做執行。
...
class Calculator extends Component {
  ...
  parentChangetoC = temp => { //子元件提供 temp，寫回到父元件的 state 上
    this.setState({
      temperature: temp,
      flagFC: 'toC'
    });
...
## Component 組合模型應用
使用元件進行組合時，透過 JSX 進行直接組合。前面介紹的用法都是直接使用`<元素名稱/>`來應用，並允許添加屬性做成 props 進行傳遞。事實上也可以透過結尾標籤來應用組合。透過此方式你不需要利用 class 繼承其他 class 獲得資源之引用問題。
...
### childeren 內容版型
包覆在元素標籤的內容也是 props 的一種，可以透過 props.children 一併提供給子元件使用。以下例示範：
...
透過 children 可以設計出一個父元件能根據子元件 A1 跟 A2 不同 Layout 版型需求而提供不同的 children。

### 多個內容版型
如果內容包覆的有多個因此一個 children 不夠用時。可以改用 props 屬性來指定不同版型元件同時傳遞多個版型。
