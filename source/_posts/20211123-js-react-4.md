...
# ref 
典型的 React 操作設計，會利用 props 來綁定上下層組件，並透過更改 props 值來重新渲染 render 使畫面更新。如果不想使用此資料流的操作，可以利用 refs 觀念跳脫此觀念。透過 ref 對特定的 JSX 元素或組件打上標記，就能透過 current 來找到這個對象做系列操作，就不用透過 props 完成互動。
...
## 向 DOM 元素添加 Ref
refs 可對 render 內的 JSX 元素打上標記，就能透過 current 獲得 JavaScript 原生的 DOM 節點進行改變操作。React 會自動偵測該 DOM 節點的變化產生渲染而不是透過 props 來偵測。有以下步驟與觀念操作：
...
## 向別的 Class Component 添加 Ref
與前者步驟相同但差別用途於在 class component 上時，則是獲得這個 class 的物件實例 instance。

>注意你只能對 Class 組件標記 ref，無法對 function 組件做標 ref 記（因為沒有 instance 可做為 current 目標）。
