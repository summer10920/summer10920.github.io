...
# 拆解成 Component
第一步先將視覺搞評估分解出有那些 React 元件做成 UI。如果設計師已提供的 PhotoShop 圖層名稱也可以拿來當作元件名稱。思考如何分解時根據 Single-responsibility principle(SRP) 單一職責原則將一個元件只負責一件事就好。這裡可分析出使用到 5 組 Componet 以及層級架構如下：
...
> ProductTable 的簡易需求之總標題，在此範例教學將被歸類在此元件內。是否根據 SRP 原則另外獨立成一個 Componet 隨人而異，如果這個總標題的功能結構龐大複雜，就適合另外獨立一個名為 ProductTableHeader 的元件。

# 規劃靜態畫面
開始建立靜態且無互動性的 render 資料模型。注意以下事項：

1. 先建立出 HTML 示意版型，在將版型打散到各元件去。
2. 盡可能地重複利用小型元件，並透過 props 來傳遞資料。
3. 不要使用 state，state 保留給互動使用。
4. 元件編寫此例建議順序為由上至下（簡單應用），如果是大型專案開發則可考慮從下至上從零件開始設計並隨時測試元件。
5. 資料從最上層開始傳遞進去，如有需要在分配給子元件透過 props 來傳遞。
...
整個範例的元件，並資料從這裡開始傳入。而根據前面分析這裡會包 SearchBar 與 ProductTable 元件。
...
## SearchBar
使用者輸入的元件。這裡先不做互動性，只單純提供 render 靜態介面部分。
...
- 需要優先判斷資料的分類。慶幸資料的順序根據分類已排列完畢，因此透過 foreach 當下順便檢查每筆資料的分類變化當下時額外進行 ProductCategoryRow 的元件輸出即可。
- 注意由於最終會直接 JSX 渲染標籤陣列，因此陣列內的標籤需要綁訂 key 提供 React 做識別。key 可以從資料差異性尋找替代，或者先前提到的第三方 Nano ID 套件來產生隨機碼。
- `<tr>`細節從下層元件來負責，讓這層的標籤陣列單純批次執行即可。
- 下層的 ProductCategoryRow 元件需要告知分類名，ProductRow 需要告知單筆商品資料，皆透過 props 傳遞。
...
## ProductCategoryRow
分類標題的元件，工作只要將接收的 props 渲染成版型出來。
...
## ProductRow
產品列的元件。額外需判別 stocked 值來給予行內樣式紅字效果。由於 JSX 對於 style 屬性有反應，不建議以字串來指定，而改物件寫法來賦予 style 值。
...
1. 由於這兩個 component 都需要用到這 2 組 state，因此皆提升到父元件 FilterableProductTable 內來創立初始化。
2. 透過標籤屬性將 state 傳遞給 ProductTable 與 SearchBar 作為 props。
3. SearchBar 元件部分，將 props（父元件的 state) 指定給自己的表單 2 組 value。
4. ProductTable 元件部分，調整 foreach 作業流程，使得產生的標籤陣列有所變化：
...
## 反向資料流
現在試著將 state 修改的能力往下傳遞，透過 porps 形成可反向的資料流動。這裡只有 SearchBar 會需要修改父元件的 state。

1. 在父元件 FilterableProductTable 規劃兩個 event 事件 dofilterTextChange 與 doinStockOnlyChange，其中參數 temp 屆時會由下層 SearchBar 提供回來。
2. 同上，透過 props 傳遞事件函式給下層 SearchBar。
3. 在子元件 SearchBar 也規劃兩個同名 event 事件 dofilterTextChange 與 doinStockOnlyChange，其中參數 e 為用戶操作下的 event 參數，將 event 參數執行在 props 傳來的父函式上，形成向上層資料傳遞。
