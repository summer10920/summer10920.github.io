...
php 本身的函式非常多，譬如之前所用的`rand()`就是其中一種。同時常有重複的函式功能，譬如`mt_rand()`跟`rand()`。你不需要特別去背函式庫，只需要用到時就去查，慢慢常用的就會記住。請開始在 Web 目錄下新增檔案 **2_array.php**，為了簡化程式碼結構將不建置 HTML 基本結構。請開始做 PHP 各種練習並預覽網頁結果。
