...
前端的 API 是指用於在網頁應用程式中通過瀏覽器與伺服器進行溝通的程式接口。API 可以是一個單獨的 JavaScript 函式，也可以是一組由伺服器提供的 RESTful API 端點。
...
- read
可以使用 read 選項來指定要注入的提供者。如果指定了這個選項，則 ViewChild 會查找該提供者，而不是使用元素本身的類型。這對於需要在元件中訪問服務或其他提供者的情況非常有用。
- static ( Angular8+ 必填）
...
我們需要透過 ComponentFactoryResolver 來建立一個元件工廠（你需要在建構子宣告），ComponentFactoryResolver 是 Angular 中一個重要的服務，它允許動態創建和載入元件，並且在執行時期動態創建這些元件，可以滿足各種動態元件的需要。
